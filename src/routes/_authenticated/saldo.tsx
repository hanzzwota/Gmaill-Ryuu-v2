...
export const Route = createFileRoute("/_authenticated/saldo")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Lihat saldo, riwayat transaksi, dan ajukan penarikan dana." },
...
const toneOf = (s: string) =>
  s === "PAID" ? "primary" : s === "REJECTED" ? "danger" : "warning";
...
function SaldoPage() {
  ...
  return (
    <div className="space-y-5">
      <SectionTitle title="Saldo & Penarikan" subtitle="Ajukan pencairan dan pantau mutasi saldo." />

      <div className="grid gap-3 sm:grid-cols-3">
        <NeoCard className="bg-primary text-primary-foreground">
          <p className="font-display text-[11px] font-bold uppercase tracking-widest">Saldo Tersedia</p>
        ...
        <NeoCard>
          ...
          <p className="neo-heading mt-1 text-2xl">{formatRp(boot?.settings.min_withdrawal ?? 0)}</p>
        </NeoCard>
      </div>
